...
<div class="main__container--table title_table">
    ...
    <form class="formulario" action="" method="GET">
        ...
        <div class="main__container--title">
            <h1>Mostrar giros</h1>
            <p>Giros actualmente dados de alta</p>
        </div>

        <table class="table-4-col tableMostrarGirosAdmin">
            <tr>
            ...
            if (isset($_GET['pagina'])) {
                if ($_GET["pagina"] == 0) {
                    header("Location:main_content/mostrarGiro.php");
                } else {
                    $pagina = $_GET['pagina'];
                }
            } else {
            ...
            ///////////////////////////////////////////////////////////////////////////////7
            // Sentencia sql
            $sql_limite = "SELECT * FROM giros ORDER BY giro_comercial LIMIT $empezar_desde, $tamano_paginas";
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo "<input class='hidden' id='id_giro" . $n . "' value='" . $row['id_giro'] . "'></input>";
...
<script>
    ...
    function mostrar($i) {
        ...
        $.ajax({
            url: "main_content/mostrarGiros.php?pagina=" + page.name,
    ...
    // Botón Editar
    function editar($i) {
        let id_giro = document.getElementById('id_giro' + $i);
        console.log(id_giro.value);

        var dato = $(this).attr("code-val");
        $.ajax({
            url: "main_content/editarGiro.php?id_giro=" + id_giro.value,
            success: function(details) {
                $("#details").html(details);
            }
        })
    ...
    function eliminar($i) {
        ...
        var dato = $(this).attr("code-val");
        $.ajax({
            url: "main_content/eliminarGiro.php?id_giro=" + id_giro.value,
            success: function(details) {
                $("#details").html(details);
            }
        })
